...
if (NOT CODE_COVERAGE_ADDED)
    ...
    if (NOT EXISTS ${CMAKE_COVERAGE_OUTPUT_DIRECTORY})
        message(STATUS "Creating coverage output directory: ${CMAKE_COVERAGE_OUTPUT_DIRECTORY}")
        file(MAKE_DIRECTORY ${CMAKE_COVERAGE_OUTPUT_DIRECTORY})
    else ()
        file(REMOVE ${CMAKE_COVERAGE_OUTPUT_DIRECTORY}/profraw.list)
        file(REMOVE ${CMAKE_COVERAGE_OUTPUT_DIRECTORY}/binaries.list)
    endif ()
    ...
    if (Python3_FOUND)
        ...
        execute_process(COMMAND Python3::Interpreter -c "import lcov_cobertura" RESULT_VARIABLE EXIT_CODE OUTPUT_QUIET)
        if (NOT (EXIT_CODE AND NOT EXIT_CODE STREQUAL 0))
            message(FATAL "Cannot find lcov_cobertura python package!")
        endif ()
    endif ()
    ...
    if (CMAKE_C_COMPILER_ID MATCHES "(Apple)?[Cc]lang" OR CMAKE_CXX_COMPILER_ID MATCHES "(Apple)?[Cc]lang")
        ...
        add_custom_target(ccov-clean
                COMMAND ${CMAKE_COMMAND} -E remove "${CMAKE_COVERAGE_OUTPUT_DIRECTORY}/profraw.list"
                COMMAND ${CMAKE_COMMAND} -E remove "${CMAKE_COVERAGE_OUTPUT_DIRECTORY}/binaries.list")
...
function(target_code_coverage TARGET_NAME)
    ...
    elseif (CMAKE_C_COMPILER_ID MATCHES "GNU" OR CMAKE_CXX_COMPILER_ID MATCHES "GNU")
        target_link_options(${TARGET_NAME} ${TARGET_VISIBILITY} --coverage -fprofile-arcs -ftest-coverage)
        target_compile_options(${TARGET_NAME} ${TARGET_VISIBILITY} --coverage -fprofile-arcs -ftest-coverage)
    endif ()
    ...
    if (target_type STREQUAL "EXECUTABLE")
        if (CMAKE_C_COMPILER_ID MATCHES "(Apple)?[Cc]lang" OR CMAKE_CXX_COMPILER_ID MATCHES "(Apple)?[Cc]lang")
            ...
            add_custom_target(ccov-${TARGET_NAME}
                    COMMAND
                    ${CMAKE_COMMAND} -E env LLVM_PROFILE_FILE=${TARGET_NAME}.profraw $<TARGET_FILE:${TARGET_NAME}> ${target_code_coverage_ARGS}
                    COMMAND
                    ${CMAKE_COMMAND} -E echo "-object=$<TARGET_FILE:${TARGET_NAME}>" ${SO_OBJECTS} >> ${CMAKE_COVERAGE_OUTPUT_DIRECTORY}/binaries.list
                    COMMAND
        ...
        elseif (CMAKE_C_COMPILER_ID MATCHES "GNU" OR CMAKE_CXX_COMPILER_ID MATCHES "GNU")
            ...
            add_custom_target(ccov-report-${TARGET_NAME}
                    COMMAND
                    ${GCOVR_PATH} -r ${CMAKE_SOURCE_DIR} "${CMAKE_BINARY_DIR}/CMakeFiles/${TARGET_NAME}.dir"
                    DEPENDS ccov-${TARGET_NAME})
