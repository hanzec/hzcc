...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%% v閞ification des pr閞equis %%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\def\loi_checkprimitive#1#2#3{% V閞ifie que #1 est une primitive et sinon, 閙et le message #2 et ex閏ute #3
	\begingroup
...
\long\def\loi_ifcsexpandable#1{% #1 est-il constitu� d'une seule sc _d関eloppable_ ?
	\loi_ifempty{#1}
		{\loi_second
		}
		{\loi_ifspacefirst{#1}
			...
			{\csname loi_\if\loi_escapechar\expandafter\loi_firsttonil\detokenize{#1}\_nil first\else second\fi\endcsname
				{\loi_exparg\loi_ifempty{\loi_gobarg#1}% 1 seul arg commen鏰nt par "\" ?
					{\def\loi_tempa{#1}\loi_exparg{\def\loi_tempb}{#1}% est-il d関eloppable ?
...
\def\loi_def_foreachsep#1{%
	...
	\long\def\loi_foreach_a##1##2#1{%
		...
		\loi_ifx{\end_foreach##1}
			{}
			{\csname loop_code_\number\loi_cnt_foreach_nest\endcsname% ex閏ute le code
			\loi_foreach_a##1%
			}%
	}%
}
...
\long\def\loi_count_occur#1\in#2:#3{% compte le nombre d'occurrences de #1 dans #2 et met le r閟ultat dans la macro #3
...
\long\def\loi_grabpaired_expr#1#2#3#4#5{% #1=liste de paires  #2=expression  #3=s閜arateur   #4=r閟ultat    #5=ce qui reste
	\let#4\empty
	\def\loi_remain{#2#3}%
...
\long\def\loi_grabpaired_expr_a#1#2#3#4{% #1#2=paire en cours  #3=s閜arateur   #4=r閟ultat
	\loi_exparg{\loi_check_pair#1#2\in}#4% si les paires sont appari閑s dans le r閟ultat
		{}% passer � la paire suivante
		{\long\def\loi_grabpaired_expr_b##1#3##2\_nil{%
			\loi_addtomacro#4{##1#3}% ajouter au r閟ultat ce qui est jusqu'au prochain s閜arateur
			\def\loi_remain{##2}%
...
\def\insidepair#1#2#3#4{% #1#2=paire  #3=expr  #4=macro re鏴vant le resultat
	\loi_if_validpair#1#2%
		{\loi_ifcsexpandable{#3}
			{\loi_exparg{\insidepair#1#2}{#3}#4%
			}
			{\loi_check_pair#1#2\in{#3}% si les paires sont appari閑s dans le r閟ultat
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%% macro retirant les espaces extr阭es %%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\long\def\setsepchar_b[#1]#2{% #1=sepcar de <liste des sepcar>  #2=<liste des sepcar>
	\loi_ifempty{#1}
		...
		\loi_nestcnt1 % r閕nitaliser niveau initial � 1
		\def\nestdepth{1}%
		...
		\loi_argcsname\let\loi_currentsep{loi_listofsep[1]}% 1er car de s閜aration
...
\def\loi_normalizeindex_a#1#2#3#4#5,{% #1=compteur de profondeur #2=index pr閏閐ents  #3=profondeur max #4=macroname  #5=index courant
	\loi_ifx{\loi_quark#5}
		{\loi_normalizeindex_c#2\loi_quark% supprimer la derni鑢e virgule
		}
		{\loi_ifnum{#1>#3 }
...
\def\loi_normalizeindex_b#1#2#3{\loi_exparg\loi_normalizeindex_a{\number\numexpr#2+1}{#3#1,}}% #1=index � rajouter  #2=compteur de profondeur #3=index pr閏閐ents
...
\def\loi_invalidindex_b#1[.]#2\loi_quark#3,\loi_quark#4\loi_quark,{{#1[#3]#2}{#3}}% #4= index ignor閟
...
\long\def\readlist_b#1#2{% #1=macro stockant les 閘閙ents  #2=liste des 閘閙ents
...
\long\def\readlist_c#1#2#3{% #1=macro stockant les 閘閙ents  #2=liste des 閘閙ents #3=macroname
	\loi_argcsname\loi_let{#3nest}\nestdepth
	\loi_argcsname\loi_def{#3[]}{#2}% la liste enti鑢e
	\loi_argcsname\loi_def{#3sep[]}{}% s閜arateur vide
	\loi_ifempty{#2}
		{\loi_def#1[##1]{}%
...
\long\def\readlist_f#1||#2\_nil#3{\readlist_g1{#3#1}}% #1=<sep courant simple>  #3=liste -> rajoute un 閘閙ent vide pour le test \ifempty ci dessous
\long\def\readlist_g#1#2{% #1=compteur d'index  #2=liste d'閘閙ents � examiner termin閑 par <sep courant simple> >>RIEN laiss� apr鑣
	\loi_ifempty{#2}
...
\long\def\readlist_h#1#2#3||{% #1=liste restante   #2=<dernier sep utilis�>  #3=<sep courant>
	\loi_ifx{\loi_quark#3}% on a 閜uis� tous les <s閜arateurs> ? RESTE � lire <expr+sep1>\_nil{<compteur>}
		{\loi_ifempty{#2}% si #2 vide, aucun <sep utilis�> n'a 閠� trouv�, il reste � lire "<liste compl鑤e>\_nil"
			{\long\def\readlist_i##1\_nil##2{\loi_exparg{\readlist_j{##2}{}}{\loi_gobarg##1}{#2}}% ##2=compteur d'index
			}
			{\loi_ifx{\loi_listofpair\empty}% paires d閒inies ?
		...
		\readlist_i\relax% le \relax meuble l'argument d閘imit�
		}
		{\long\def\readlist_i##1#3##2\_nil{%
			\loi_ifempty{##2}% si <liste restante> ne contient pas le <sep courant>
				{\readlist_h{#1}{#2}% recommencer avec le m阭e <sep utile>
				}%
				{\loi_ifx{\loi_listofpair\empty}% si pas de paires d閒inies
					...
					\loi_ifx{\loi_grabpaired_remain\empty}% si liste non raccourcie #BUGFIX 1.63
						{\loi_exparg\readlist_h{\loi_grabpaired_result}{#2}}% garder le pr閏閐ent <sep>
...
\long\def\readlist_j#1#2#3{% #1=compteur d'index  #2=liste restante  #3=閘閙ent courant
	\loi_ifnum{0\loi_exparg\loi_ifspacefirst{\loi_currentsep}{}1\if_removeextremespaces1\fi=11 }% s'il faur retirer les espaces extr阭es
		{\loi_exparg{\loi_exparg{\readlist_k{#1}{#2}}}{\loi_removeextremespaces{#3}}}% red閒inir l'閘閙ent courant
		{\readlist_k{#1}{#2}{#3}}%
}
\long\def\readlist_k#1#2#3#4{% #1=compteur d'index  #2=liste restante  #3=閘閙ent courant   #4=sep utilis�
	\loi_ifnum{0\if_ignoreemptyitems1\fi\loi_ifempty{#3}1{}=11 }
		{\readlist_g{#1}{#2}% si l'on n'ignore pas les 閘閙ents vides
		}%
		...
		\loi_ifnum{\loi_nestcnt<\nestdepth\relax}% si imbrication max non atteinte
			...
			\readlist_e{#3}% recommencer avec l'閘閙ent courant
			}
			{}%
		\loi_exparg\readlist_g{\number\numexpr#1+1}{#2}% puis chercher l'閘閙ent suivant dans la liste restante
		}%
}
...
\unless\ifdefined\fbox
	\newdimen\fboxrule \newdimen\fboxsep \fboxrule=.4pt \fboxsep=3pt % r間lages identiques � LaTeX
...
\fi
\def\showitemsmacro#1{% encadrement par d閒aut
