...
ALWAYS_INLINE static bool checkAndPrint(Lexical::TokenType type,
                                        TokenList& tokens) {
    if (tokens.peek().Type() != type) {
        MYCC_PrintFirstTokenError(
            tokens,
            "Except an " +
                Lexical::SymbolUtils::TokenTypeToString(tokens.peek().Type()) +
                " but got: " +
...
std::list<Lexical::Token> ParseTypeName(TokenList& tokens,
    ...
    if (tokens.peek().Type() == Lexical::TokenType::kStruct ||
        tokens.peek().Type() == Lexical::TokenType::kUnion) {
        is_struct = true;
    ...
    if ((tokens.peek().Type() == Lexical::TokenType::kIdentity &&
         context.hasType(tokens.peek().Value())) ||
        tokens.peek().Type() == Lexical::TokenType::kType) {
        final_type_name.push_back(tokens.pop());
    } else if (!is_struct) {
        return {};
    }
    ...
    if (tokens.peek().Type() == Lexical::TokenType::kReference ||
        ...
        while (tokens.peek().Type() == Lexical::TokenType::kReference) {
            // consume "*"
...
ParseVariable(               // NOLINT
    ...
    if (tokens.peek().Type() == Lexical::TokenType::kLBracket) {
        ...
        if (tokens.size() >= 2 &&
            tokens.peek2().Type() == Lexical::TokenType::kRBracket) {
            tokens.pop();  // consume "]"
            tokens.pop();  // consume "["
        ...
        while (tokens.peek().Type() == Lexical::TokenType::kLBracket) {
            if (tokens.size() >= 2 &&
                tokens.peek2().Type() != Lexical::TokenType::kRBracket) {
                tokens.pop();  // consume "["
...
std::tuple<std::shared_ptr<AST::Type>, Lexical::Token> ParseTypeDecl(
    ...
    if (tokens.peek().Type() == Lexical::TokenType::kIdentity) {
        ...
        if (!array_shape.empty()) {
            ...
            auto attrs = tokens.LoadCachedAttributes();
            return std::make_tuple(context.getNamedType(type_name, attrs),
                                   variable_name);
        }
    }
     ...
     */
        ...
        if (context.hasType(type_name)) {
            auto attrs = tokens.LoadCachedAttributes();
            return std::make_tuple(
                context.getNamedType(type_name, attrs),
                Lexical::Token(Lexical::TokenType::kUnknown, -1, -1));
        } else {
