...
Lexical::Token TokenList::pop() {
    ...
    while (TokenUtils::IsAttribute(token.Type())) {
        DVLOG(SYNTAX_LOG_LEVEL)
            << "cached attribute: " << MYCC_PRETTY_PRINT_TOKEN(token);
        pop_front();
        _cached_attributes.push_back(token);
        token = front();
    }
    pop_front();
    DVLOG(SYNTAX_LOG_LEVEL) << "pop token: " << MYCC_PRETTY_PRINT_TOKEN(token);
    return token;
}

void TokenList::push(const Lexical::Token& token) {
    DVLOG(SYNTAX_LOG_LEVEL) << "push token: " << MYCC_PRETTY_PRINT_TOKEN(token);
    emplace_front(token);
}
...
Lexical::Token TokenList::peek() {
    while (TokenUtils::IsAttribute(front().Type())) {
        DVLOG(SYNTAX_LOG_LEVEL)
            << "cached attribute: " << MYCC_PRETTY_PRINT_TOKEN(front());
        auto token = front();
        pop_front();
        _cached_attributes.push_back(token);
    }
