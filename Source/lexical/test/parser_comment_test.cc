...
inline __attribute__((always_inline)) void test_line_1(int line_num, int row_offset,
  ...
  ASSERT_EQ(std::make_pair(line_num, row_offset + 0), result.front().Location());
  result.pop_front();

  ASSERT_EQ("a", result.front().Value());
  ASSERT_EQ(kIdentity, result.front().Type());
  ASSERT_EQ(std::make_pair(line_num, row_offset + 4), result.front().Location());
  result.pop_front();

  ASSERT_EQ("=", result.front().Value());
  ASSERT_EQ(kAssign, result.front().Type());
  ASSERT_EQ(std::make_pair(line_num, row_offset + 6), result.front().Location());
  result.pop_front();

  ASSERT_EQ("1", result.front().Value());
  ASSERT_EQ(kInteger, result.front().Type());
  ASSERT_EQ(std::make_pair(line_num, row_offset + 8), result.front().Location());
  result.pop_front();

  ASSERT_EQ(";", result.front().Value());
  ASSERT_EQ(kSemiColon, result.front().Type());
  ASSERT_EQ(std::make_pair(line_num, row_offset + 9), result.front().Location());
  result.pop_front();
}
...
TEST(LEXICAL_PARSER, c_style_comment_3)
{// NOLINT
  std::stringstream os;
  os << "// This is a comment\n";
  os << "int a = 1;\n";
