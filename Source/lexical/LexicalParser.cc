      ...
      switch (line_buf[col]) {
        ...
        case '/': {  // handling comment
          if (line_buf[col + 1] == '/') {
            ...
            // a special case where multiple line comment is used as single line comment
        ...
        case '"': {  // handling string literal
          ...
          if (std::string::npos == col) {
            ...
            while (col != std::string::npos &&
                   (line_buf[col - 1] == '\\' || line_buf[col] != '\"')) {
              if (col - 2 >= 0 && line_buf[col - 2] == '\\') {
                /*
                 * Cases like ["This is evil \\"] where second \ is already escaped by first \
                 */
                break;
              }
            ...
            // if not found other '"' until end of line after escape sequence
            if (std::string::npos == col) {
              have_error = true;
              Message::print_message(Message::kError, "unclosed string literal", line,
                                     source_name, std::make_pair(row, start - 1));
        ...
        default: {
          if (std::isdigit(line_buf[col])) {
            ...
            if (col != std::string::npos) {
              ...
              if (std::tolower(line_buf[col]) == 'b' || std::tolower(line_buf[col]) == 'x') {
                col++;
              }
            ...
            if (type == LexicalToken::kInteger) {
              ...
              try {
                if (std::stod(tmp_numbers) > std::numeric_limits<long double>::max() ||
              ...
              if ((tmp_numbers.length() - dot_pos) >= kMaxSignificantDigits) {
                tmp_numbers.erase(dot_pos + kMaxSignificantDigits + 1, tmp_numbers.length());
          ...
          HANDLE_SYMBOL:
            ...
            /**
             * Here is not add the second symbol to token stream in this loop is because
             * there is a special situation where the comment symbol is after the first
             * symbol like this:
             *     [int a = 1; // comment] or [int a = 1; /\* start comment *\/]
             * Which could cause parsing error, there is no need to insert special logic
             * here since only redo one symbol would not influence the performance very
             * much.
             */
            if (type > 300) {  // symbols except single char will have value larger then 300
              col++;           // back to the last digit
            }
