...
int main(int argc, char* argv[]) {
  ...
  app.add_flag("--fno_color", Mycc::Options::Global_disable_color, "Disable print color");
  ...
  app.add_option("-I,--include", included_files, "include file")->check(CLI::ExistingFile);

  // input files [THIS REQUIRED AT LEAST ONE]
  std::vector<std::string> input_files;
  app.add_option("file, -c", input_files, "Source Code")->check(CLI::ExistingFile);
  ...
  if (flag1) {
    ...
    std::vector<Mycc::Lexical::LexicalToken> tokens;

    auto lexical_result = Mycc::Lexical::ParseToToken(infile, tokens, input_files[0]);

    if (output_file.empty()) {
      for (auto& token : tokens) {
        std::cout << "File " << input_files[0] << " Line " << std::setw(5)
                  << token.Location().first + 1 << std::setw(0) << " Token " << std::setw(3)
                  << token.TokenType() << std::setw(0) << " Text " << token.Value(true)
                  << std::endl;
      }
    } else {
