...
SymbTbl::SymbTbl(QualTypePtr return_type, std::weak_ptr<SymbTbl> parent)
    : _upper_scope_table(std::move(parent)), _return_type(std::move(return_type)) {}

std::shared_ptr<SymbTbl> SymbTbl::getUpperScope() { return _upper_scope_table.lock(); }
...
void SymbTbl::add_var(Position pos,             // NOLINT
    ...
    DLOG_ASSERT(type != nullptr) << " Variable " << name << " has passed a nullptr type";
    ...
    _variable_lookup_table.emplace(std::make_pair(name, std::make_pair(pos, type)));
}
...
Position SymbTbl::var_def_pos(std::string_view name) {
    ...
    LOG_IF(FATAL, has_var(name, false)) << "Variable " << name << " has not been defined";
#endif
