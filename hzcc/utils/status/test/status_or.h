...
#ifndef HZCC_STATUS_OR_INTERNAL_H
#define HZCC_STATUS_OR_H
...
inline const ::hzcc::Status& GetStatus(const ::hzcc::Status& status) {
    return status;
}

template <typename T>
inline const ::hzcc::Status& GetStatus(const ::hzcc::StatusOr<T>& status) {
...
// Macros for testing the results of functions that return hzcc::Status or
// hzcc::StatusOr<T> (for any type T).
...
hzcc::StatusOr<std::unique_ptr<int>> ReturnUniquePtr() {
    // Uses implicit constructor from T&&
    return std::make_unique<int>(0);
}

TEST(StatusOr, ElementType) {
    static_assert(std::is_same<hzcc::StatusOr<int>::value_type, int>(), "");
    static_assert(std::is_same<hzcc::StatusOr<char>::value_type, char>(), "");
}

TEST(StatusOr, TestMoveOnlyInitialization) {
    hzcc::StatusOr<std::unique_ptr<int>> thing(ReturnUniquePtr());
...
TEST(StatusOr, TestValueOrDieOverloadForConstTemporary) {
    static_assert(
        std::is_same<
            const int&&,
            decltype(std::declval<const hzcc::StatusOr<int>&&>().value())>(),
        "value() for const temporaries should return const T&&");
}

TEST(StatusOr, TestMoveOnlyConversion) {
    hzcc::StatusOr<std::unique_ptr<const int>> const_thing(ReturnUniquePtr());
...
TEST(StatusOr, TestMoveOnlyVector) {
    // Sanity check that hzcc::StatusOr<MoveOnly> works in vector.
    std::vector<hzcc::StatusOr<std::unique_ptr<int>>> vec;
...
TEST(StatusOr, TestDefaultCtor) {
    hzcc::StatusOr<int> thing;
    EXPECT_FALSE(thing.ok());
    EXPECT_EQ(thing.status().code(), hzcc::StatusCode::kUnknown);
}

TEST(StatusOr, StatusCtorForwards) {
    hzcc::Status status(hzcc::StatusCode::kInternal, "Some error");

    EXPECT_EQ(hzcc::StatusOr<int>(status).status().message(), "Some error");
    EXPECT_EQ(status.message(), "Some error");

    EXPECT_EQ(hzcc::StatusOr<int>(std::move(status)).status().message(),
              "Some error");
...
TEST(StatusOrDeathTest, TestDefaultCtorValue) {
    ...
    const hzcc::StatusOr<int> thing2;
    EXPECT_DEATH_OR_THROW(thing2.value(), absl::UnknownError(""));
}

TEST(StatusOrDeathTest, TestValueNotOk) {
    hzcc::StatusOr<int> thing(absl::CancelledError());
    EXPECT_DEATH_OR_THROW(thing.value(), absl::CancelledError());
}

TEST(StatusOrDeathTest, TestValueNotOkConst) {
    const hzcc::StatusOr<int> thing(absl::UnknownError(""));
    EXPECT_DEATH_OR_THROW(thing.value(), absl::UnknownError(""));
}
...
TEST(StatusOrDeathTest, TestPointerValueNotOkConst) {
    const hzcc::StatusOr<int*> thing(absl::CancelledError());
    EXPECT_DEATH_OR_THROW(thing.value(), absl::CancelledError());
}
...
TEST(StatusOrDeathTest, TestStatusCtorStatusOk) {
    EXPECT_DEBUG_DEATH(
        {
            // This will DCHECK
            hzcc::StatusOr<int> thing(absl::OkStatus());
            // In optimized mode, we are actually going to get error::INTERNAL for
            // status here, rather than crashing, so check that.
            EXPECT_FALSE(thing.ok());
            EXPECT_EQ(thing.status().code(), hzcc::StatusCode::kInternal);
        },
...
TEST(StatusOr, ValueAccessor) {
    ...
    {
        hzcc::StatusOr<int> status_or(kIntValue);
        EXPECT_EQ(kIntValue, status_or.value());
        EXPECT_EQ(kIntValue, std::move(status_or).value());
    }
    {
        hzcc::StatusOr<CopyDetector> status_or(kIntValue);
...
TEST(StatusOr, BadValueAccess) {
    const hzcc::Status kError = absl::CancelledError("message");
    hzcc::StatusOr<int> status_or(kError);
...
TEST(StatusOr, TestStatusCtor) {
    hzcc::StatusOr<int> thing(absl::CancelledError());
    EXPECT_FALSE(thing.ok());
    EXPECT_EQ(thing.status().code(), hzcc::StatusCode::kCancelled);
}

TEST(StatusOr, TestValueCtor) {
    const int kI = 4;
    const hzcc::StatusOr<int> thing(kI);
    EXPECT_TRUE(thing.ok());
    EXPECT_EQ(kI, *thing);
}
...
TEST(StatusOr, InPlaceConstruction) {
    EXPECT_THAT(hzcc::StatusOr<Foo>(absl::in_place, 10),
...
TEST(StatusOr, InPlaceInitListConstruction) {
    hzcc::StatusOr<InPlaceHelper> status_or(absl::in_place, {10, 11, 12},
...
TEST(StatusOr, Emplace) {
    hzcc::StatusOr<Foo> status_or_foo(10);
    ...
    EXPECT_EQ(status_or_foo.status().code(), hzcc::StatusCode::kInvalidArgument);
...
TEST(StatusOr, EmplaceInitializerList) {
    ...
    EXPECT_EQ(status_or.status().code(), hzcc::StatusCode::kInvalidArgument);
...
TEST(StatusOr, TestCopyCtorStatusOk) {
    const int kI = 4;
    const hzcc::StatusOr<int> original(kI);
    const hzcc::StatusOr<int> copy(original);
    EXPECT_OK(copy.status());
    EXPECT_EQ(*original, *copy);
}

TEST(StatusOr, TestCopyCtorStatusNotOk) {
    hzcc::StatusOr<int> original(absl::CancelledError());
    hzcc::StatusOr<int> copy(original);
    EXPECT_EQ(copy.status().code(), hzcc::StatusCode::kCancelled);
}

TEST(StatusOr, TestCopyCtorNonAssignable) {
    const int kI = 4;
    CopyNoAssign value(kI);
    hzcc::StatusOr<CopyNoAssign> original(value);
    hzcc::StatusOr<CopyNoAssign> copy(original);
    EXPECT_OK(copy.status());
...
TEST(StatusOr, TestCopyCtorStatusOKConverting) {
    const int kI = 4;
    hzcc::StatusOr<int> original(kI);
    hzcc::StatusOr<double> copy(original);
    EXPECT_OK(copy.status());
...
TEST(StatusOr, TestAssignmentStatusOk) {
    // Copy assignmment
    {
        ...
        hzcc::StatusOr<std::shared_ptr<int>> source(p);

        hzcc::StatusOr<std::shared_ptr<int>> target;
        target = source;
...
TEST(StatusOr, TestAssignmentStatusNotOk) {
    // Copy assignment
    {
        const hzcc::Status expected = absl::CancelledError();
        hzcc::StatusOr<int> source(expected);

        hzcc::StatusOr<int> target;
        target = source;
    ...
    // Move assignment
    {
        ...
        EXPECT_FALSE(source.ok());
        EXPECT_EQ(source.status().code(), hzcc::StatusCode::kInternal);
    }
}
...
TEST(StatusOr, TestAssignmentStatusOKConverting) {
    ...
    {
        const int kI = 4;
        hzcc::StatusOr<int> source(kI);

        hzcc::StatusOr<double> target;
        target = source;
    ...
    // Move assignment
    {
        ...
        hzcc::StatusOr<std::unique_ptr<int>> source(absl::WrapUnique(p));
...
TEST(StatusOr, ImplicitConvertingConstructor) {
    EXPECT_THAT(
        absl::implicit_cast<hzcc::StatusOr<ImplicitConstructibleFromA>>(
            hzcc::StatusOr<A>(A{11})),
        IsOkAndHolds(AllOf(Field(&ImplicitConstructibleFromA::x, 11),
                           Field(&ImplicitConstructibleFromA::moved, true))));
    hzcc::StatusOr<A> a(A{12});
    EXPECT_THAT(
        absl::implicit_cast<hzcc::StatusOr<ImplicitConstructibleFromA>>(a),
...
TEST(StatusOr, ExplicitConvertingConstructor) {
    EXPECT_FALSE(
        (std::is_convertible<const hzcc::StatusOr<A>&,
                             hzcc::StatusOr<ExplicitConstructibleFromA>>::value));
    EXPECT_FALSE(
        (std::is_convertible<hzcc::StatusOr<A>&&,
                             hzcc::StatusOr<ExplicitConstructibleFromA>>::value));
    EXPECT_THAT(
        hzcc::StatusOr<ExplicitConstructibleFromA>(hzcc::StatusOr<A>(A{11})),
        IsOkAndHolds(AllOf(Field(&ExplicitConstructibleFromA::x, 11),
                           Field(&ExplicitConstructibleFromA::moved, true))));
    hzcc::StatusOr<A> a(A{12});
    EXPECT_THAT(
        hzcc::StatusOr<ExplicitConstructibleFromA>(a),
...
TEST(StatusOr, ImplicitBooleanConstructionWithImplicitCasts) {
    EXPECT_THAT(hzcc::StatusOr<bool>(hzcc::StatusOr<ConvertibleToBool>(true)),
                IsOkAndHolds(true));
    EXPECT_THAT(hzcc::StatusOr<bool>(hzcc::StatusOr<ConvertibleToBool>(false)),
                IsOkAndHolds(false));
    EXPECT_THAT(
        absl::implicit_cast<hzcc::StatusOr<ImplicitConstructibleFromBool>>(
            hzcc::StatusOr<bool>(false)),
        IsOkAndHolds(Field(&ImplicitConstructibleFromBool::x, false)));
    EXPECT_FALSE((std::is_convertible<
                  hzcc::StatusOr<ConvertibleToBool>,
                  hzcc::StatusOr<ImplicitConstructibleFromBool>>::value));
}
...
TEST(StatusOr, BooleanConstructionWithImplicitCasts) {
    ...
    EXPECT_THAT(hzcc::StatusOr<bool>(hzcc::StatusOr<ConvertibleToBool>(false)),
                IsOkAndHolds(false));
    EXPECT_THAT(
        hzcc::StatusOr<ImplicitConstructibleFromBool>{
            hzcc::StatusOr<bool>(false)},
        IsOkAndHolds(Field(&ImplicitConstructibleFromBool::x, false)));
    EXPECT_THAT(
        hzcc::StatusOr<ImplicitConstructibleFromBool>{
            hzcc::StatusOr<bool>(absl::InvalidArgumentError(""))},
        Not(IsOk()));

    EXPECT_THAT(
        hzcc::StatusOr<ImplicitConstructibleFromBool>{
            hzcc::StatusOr<ConvertibleToBool>(ConvertibleToBool{false})},
        IsOkAndHolds(Field(&ImplicitConstructibleFromBool::x, false)));
    EXPECT_THAT(
        hzcc::StatusOr<ImplicitConstructibleFromBool>{
            hzcc::StatusOr<ConvertibleToBool>(absl::InvalidArgumentError(""))},
        Not(IsOk()));
}

TEST(StatusOr, ConstImplicitCast) {
    EXPECT_THAT(absl::implicit_cast<hzcc::StatusOr<bool>>(
                    hzcc::StatusOr<const bool>(true)),
                IsOkAndHolds(true));
    EXPECT_THAT(absl::implicit_cast<hzcc::StatusOr<bool>>(
                    hzcc::StatusOr<const bool>(false)),
                IsOkAndHolds(false));
    EXPECT_THAT(absl::implicit_cast<hzcc::StatusOr<const bool>>(
                    hzcc::StatusOr<bool>(true)),
                IsOkAndHolds(true));
    EXPECT_THAT(absl::implicit_cast<hzcc::StatusOr<const bool>>(
                    hzcc::StatusOr<bool>(false)),
                IsOkAndHolds(false));
    EXPECT_THAT(absl::implicit_cast<hzcc::StatusOr<const std::string>>(
                    hzcc::StatusOr<std::string>("foo")),
                IsOkAndHolds("foo"));
    EXPECT_THAT(absl::implicit_cast<hzcc::StatusOr<std::string>>(
                    hzcc::StatusOr<const std::string>("foo")),
                IsOkAndHolds("foo"));
    EXPECT_THAT(
        absl::implicit_cast<hzcc::StatusOr<std::shared_ptr<const std::string>>>(
            hzcc::StatusOr<std::shared_ptr<std::string>>(
                std::make_shared<std::string>("foo"))),
        IsOkAndHolds(Pointee(std::string("foo"))));
}

TEST(StatusOr, ConstExplicitConstruction) {
    EXPECT_THAT(hzcc::StatusOr<bool>(hzcc::StatusOr<const bool>(true)),
                IsOkAndHolds(true));
    EXPECT_THAT(hzcc::StatusOr<bool>(hzcc::StatusOr<const bool>(false)),
                IsOkAndHolds(false));
    EXPECT_THAT(hzcc::StatusOr<const bool>(hzcc::StatusOr<bool>(true)),
                IsOkAndHolds(true));
    EXPECT_THAT(hzcc::StatusOr<const bool>(hzcc::StatusOr<bool>(false)),
                IsOkAndHolds(false));
}
...
TEST(StatusOr, ExplicitConstruction) {
    EXPECT_THAT(hzcc::StatusOr<ExplicitConstructibleFromInt>(10),
...
TEST(StatusOr, ImplicitConstruction) {
    ...
    auto status_or =
        absl::implicit_cast<hzcc::StatusOr<absl::variant<int, std::string>>>(10);
...
TEST(StatusOr, ImplicitConstructionFromInitliazerList) {
    ...
    // by hzcc::StatusOr or absl::optional.
    auto status_or =
        absl::implicit_cast<hzcc::StatusOr<std::vector<int>>>({{10, 20, 30}});
    EXPECT_THAT(status_or, IsOkAndHolds(ElementsAre(10, 20, 30)));
}

TEST(StatusOr, UniquePtrImplicitConstruction) {
    auto status_or = absl::implicit_cast<hzcc::StatusOr<std::unique_ptr<Base1>>>(
        std::make_unique<Derived>());
    EXPECT_THAT(status_or, IsOkAndHolds(Ne(nullptr)));
}

TEST(StatusOr, NestedStatusOrCopyAndMoveConstructorTests) {
    hzcc::StatusOr<hzcc::StatusOr<CopyDetector>> status_or = CopyDetector(10);
    hzcc::StatusOr<hzcc::StatusOr<CopyDetector>> status_error =
        absl::InvalidArgumentError("foo");
    EXPECT_THAT(status_or,
                IsOkAndHolds(IsOkAndHolds(CopyDetectorHas(10, true, false))));
    hzcc::StatusOr<hzcc::StatusOr<CopyDetector>> a = status_or;
    EXPECT_THAT(a, IsOkAndHolds(IsOkAndHolds(CopyDetectorHas(10, false, true))));
    hzcc::StatusOr<hzcc::StatusOr<CopyDetector>> a_err = status_error;
    EXPECT_THAT(a_err, Not(IsOk()));

    const hzcc::StatusOr<hzcc::StatusOr<CopyDetector>>& cref = status_or;
    hzcc::StatusOr<hzcc::StatusOr<CopyDetector>> b = cref;  // NOLINT
    EXPECT_THAT(b, IsOkAndHolds(IsOkAndHolds(CopyDetectorHas(10, false, true))));
    const hzcc::StatusOr<hzcc::StatusOr<CopyDetector>>& cref_err = status_error;
    hzcc::StatusOr<hzcc::StatusOr<CopyDetector>> b_err = cref_err;  // NOLINT
    EXPECT_THAT(b_err, Not(IsOk()));

    hzcc::StatusOr<hzcc::StatusOr<CopyDetector>> c = std::move(status_or);
    EXPECT_THAT(c, IsOkAndHolds(IsOkAndHolds(CopyDetectorHas(10, true, false))));
    hzcc::StatusOr<hzcc::StatusOr<CopyDetector>> c_err = std::move(status_error);
...
TEST(StatusOr, NestedStatusOrCopyAndMoveAssignment) {
    ...
    hzcc::StatusOr<hzcc::StatusOr<CopyDetector>> status_error =
        absl::InvalidArgumentError("foo");
    hzcc::StatusOr<hzcc::StatusOr<CopyDetector>> a;
    a = status_or;
    EXPECT_THAT(a, IsOkAndHolds(IsOkAndHolds(CopyDetectorHas(10, false, true))));
    a = status_error;
...
TEST(StatusOr, StatusOrAnyCopyAndMoveConstructorTests) {
    hzcc::StatusOr<absl::any> status_or = CopyDetector(10);
    hzcc::StatusOr<absl::any> status_error = absl::InvalidArgumentError("foo");
    EXPECT_THAT(
        status_or,
        IsOkAndHolds(AnyWith<CopyDetector>(CopyDetectorHas(10, true, false))));
    hzcc::StatusOr<absl::any> a = status_or;
    EXPECT_THAT(
        a, IsOkAndHolds(AnyWith<CopyDetector>(CopyDetectorHas(10, false, true))));
    hzcc::StatusOr<absl::any> a_err = status_error;
    EXPECT_THAT(a_err, Not(IsOk()));

    const hzcc::StatusOr<absl::any>& cref = status_or;
    // No lint for no-change copy.
    hzcc::StatusOr<absl::any> b = cref;  // NOLINT
    EXPECT_THAT(
        b, IsOkAndHolds(AnyWith<CopyDetector>(CopyDetectorHas(10, false, true))));
    const hzcc::StatusOr<absl::any>& cref_err = status_error;
    // No lint for no-change copy.
    hzcc::StatusOr<absl::any> b_err = cref_err;  // NOLINT
    EXPECT_THAT(b_err, Not(IsOk()));

    hzcc::StatusOr<absl::any> c = std::move(status_or);
    EXPECT_THAT(
        c, IsOkAndHolds(AnyWith<CopyDetector>(CopyDetectorHas(10, true, false))));
    hzcc::StatusOr<absl::any> c_err = std::move(status_error);
    EXPECT_THAT(c_err, Not(IsOk()));
}
...
TEST(StatusOr, StatusOrCopyAndMoveTestsConstructor) {
    hzcc::StatusOr<CopyDetector> status_or(10);
    ASSERT_THAT(status_or, IsOkAndHolds(CopyDetectorHas(10, false, false)));
    hzcc::StatusOr<CopyDetector> a(status_or);
    EXPECT_THAT(a, IsOkAndHolds(CopyDetectorHas(10, false, true)));
    const hzcc::StatusOr<CopyDetector>& cref = status_or;
    hzcc::StatusOr<CopyDetector> b(cref);  // NOLINT
    EXPECT_THAT(b, IsOkAndHolds(CopyDetectorHas(10, false, true)));
    hzcc::StatusOr<CopyDetector> c(std::move(status_or));
...
TEST(StatusOr, StatusOrCopyAndMoveTestsAssignment) {
    hzcc::StatusOr<CopyDetector> status_or(10);
    ASSERT_THAT(status_or, IsOkAndHolds(CopyDetectorHas(10, false, false)));
    hzcc::StatusOr<CopyDetector> a;
    a = status_or;
    EXPECT_THAT(a, IsOkAndHolds(CopyDetectorHas(10, false, true)));
    const hzcc::StatusOr<CopyDetector>& cref = status_or;
    hzcc::StatusOr<CopyDetector> b;
    b = cref;
    EXPECT_THAT(b, IsOkAndHolds(CopyDetectorHas(10, false, true)));
    hzcc::StatusOr<CopyDetector> c;
    c = std::move(status_or);
    EXPECT_THAT(c, IsOkAndHolds(CopyDetectorHas(10, true, false)));
}

TEST(StatusOr, AbslAnyAssignment) {
    EXPECT_FALSE((std::is_assignable<hzcc::StatusOr<absl::any>,
                                     hzcc::StatusOr<int>>::value));
    hzcc::StatusOr<absl::any> status_or;
    status_or = absl::InvalidArgumentError("foo");
    EXPECT_THAT(status_or, Not(IsOk()));
}

TEST(StatusOr, ImplicitAssignment) {
    hzcc::StatusOr<absl::variant<int, std::string>> status_or;
    status_or = 10;
    EXPECT_THAT(status_or, IsOkAndHolds(VariantWith<int>(10)));
}

TEST(StatusOr, SelfDirectInitAssignment) {
    hzcc::StatusOr<std::vector<int>> status_or = {{10, 20, 30}};
    status_or = *status_or;
    EXPECT_THAT(status_or, IsOkAndHolds(ElementsAre(10, 20, 30)));
}
...
TEST(StatusOr, UniquePtrImplicitAssignment) {
    hzcc::StatusOr<std::unique_ptr<Base1>> status_or;
...
TEST(StatusOr, Pointer) {
    ...
    EXPECT_TRUE((std::is_constructible<hzcc::StatusOr<A*>, B*>::value));
    EXPECT_TRUE((std::is_convertible<B*, hzcc::StatusOr<A*>>::value));
    EXPECT_FALSE((std::is_constructible<hzcc::StatusOr<A*>, C*>::value));
    EXPECT_FALSE((std::is_convertible<C*, hzcc::StatusOr<A*>>::value));
}
...
TEST(StatusOr, SelfAssignment) {
    // Copy-assignment, status OK
    {
        ...
        hzcc::StatusOr<std::string> so = long_str;
        so = *&so;
    ...
    {
        hzcc::StatusOr<int> so = absl::NotFoundError("taco");
        so = *&so;

        EXPECT_FALSE(so.ok());
        EXPECT_EQ(so.status().code(), hzcc::StatusCode::kNotFound);
        EXPECT_EQ(so.status().message(), "taco");
    }

    // Move-assignment with copyable type, status OK
    {
        hzcc::StatusOr<int> so = 17;
    ...
    // Move-assignment with non-copyable type, status OK
    {
        ...
        hzcc::StatusOr<std::unique_ptr<int>> so = absl::WrapUnique(raw);
    ...
    {
        hzcc::StatusOr<std::unique_ptr<int>> so = absl::NotFoundError("taco");
...
TEST(StatusOr, PerfectForwardingAssignment) {
    ...
    hzcc::StatusOr<CopyDetector> status_or;
    ...
    // U != T
    EXPECT_TRUE(
        (std::is_assignable<hzcc::StatusOr<MockValue>&,
                            const FromConstructibleAssignableLvalue&>::value));
    EXPECT_TRUE((std::is_assignable<hzcc::StatusOr<MockValue>&,
    ...
    EXPECT_FALSE((std::is_assignable<hzcc::StatusOr<MockValue>&,
                                     const FromAssignableOnly&>::value));

    hzcc::StatusOr<MockValue> from_lvalue(FromConstructibleAssignableLvalue{});
    ...
    hzcc::StatusOr<MockValue> from_rvalue(FromConstructibleAssignableRvalue{});
    ...
    hzcc::StatusOr<MockValue> from_implicit_constructible(
...
TEST(StatusOr, TestStatus) {
    hzcc::StatusOr<int> good(4);
    EXPECT_TRUE(good.ok());
    hzcc::StatusOr<int> bad(absl::CancelledError());
    EXPECT_FALSE(bad.ok());
    EXPECT_EQ(bad.status().code(), hzcc::StatusCode::kCancelled);
}

TEST(StatusOr, OperatorStarRefQualifiers) {
    static_assert(
        std::is_same<const int&,
                     decltype(*std::declval<const hzcc::StatusOr<int>&>())>(),
        "Unexpected ref-qualifiers");
    static_assert(
        std::is_same<int&, decltype(*std::declval<hzcc::StatusOr<int>&>())>(),
        "Unexpected ref-qualifiers");
    static_assert(
        std::is_same<const int&&,
                     decltype(*std::declval<const hzcc::StatusOr<int>&&>())>(),
...
TEST(StatusOr, OperatorStar) {
    const hzcc::StatusOr<std::string> const_lvalue("hello");
    EXPECT_EQ("hello", *const_lvalue);

    hzcc::StatusOr<std::string> lvalue("hello");
    EXPECT_EQ("hello", *lvalue);

    // Note: Recall that std::move() is equivalent to a static_cast to an rvalue
    // reference type.
    const hzcc::StatusOr<std::string> const_rvalue("hello");
    EXPECT_EQ("hello", *std::move(const_rvalue));  // NOLINT

    hzcc::StatusOr<std::string> rvalue("hello");
...
TEST(StatusOr, OperatorArrowQualifiers) {
    static_assert(
        std::is_same<
            const int*,
            decltype(std::declval<const hzcc::StatusOr<int>&>().operator->())>(),
        "Unexpected qualifiers");
    static_assert(
        std::is_same<
            int*, decltype(std::declval<hzcc::StatusOr<int>&>().operator->())>(),
        "Unexpected qualifiers");
    static_assert(
        std::is_same<
            const int*,
...
TEST(StatusOr, RValueStatus) {
    hzcc::StatusOr<int> so(absl::NotFoundError("taco"));
    const hzcc::Status s = std::move(so).status();

    EXPECT_EQ(s.code(), hzcc::StatusCode::kNotFound);
    ...
    EXPECT_EQ(so.status().code(), hzcc::StatusCode::kInternal);
    EXPECT_EQ(so.status().message(), "Status accessed after move.");
}

TEST(StatusOr, TestValue) {
    const int kI = 4;
    hzcc::StatusOr<int> thing(kI);
    EXPECT_EQ(kI, *thing);
}
...
TEST(StatusOr, TestPointerValueCtor) {
    ...
    {
        hzcc::StatusOr<const int*> so(nullptr);
        EXPECT_TRUE(so.ok());
        EXPECT_OK(so.status());
...
TEST(StatusOr, TestPointerCopyCtorStatusOk) {
    const int kI = 0;
    hzcc::StatusOr<const int*> original(&kI);
    hzcc::StatusOr<const int*> copy(original);
    EXPECT_OK(copy.status());
    EXPECT_EQ(*original, *copy);
}
...
TEST(StatusOr, TestPointerCopyCtorStatusOKConverting) {
    Derived derived;
    hzcc::StatusOr<Derived*> original(&derived);
    hzcc::StatusOr<Base2*> copy(original);
    EXPECT_OK(copy.status());
    EXPECT_EQ(static_cast<const Base2*>(*original), *copy);
}

TEST(StatusOr, TestPointerCopyCtorStatusNotOkConverting) {
    hzcc::StatusOr<Derived*> original(absl::CancelledError());
    hzcc::StatusOr<Base2*> copy(original);
    EXPECT_EQ(copy.status().code(), hzcc::StatusCode::kCancelled);
}
...
TEST(StatusOr, TestPointerAssignmentStatusOk) {
    const int kI = 0;
    hzcc::StatusOr<const int*> source(&kI);
    hzcc::StatusOr<const int*> target;
    target = source;
    EXPECT_OK(target.status());
...
TEST(StatusOr, TestPointerAssignmentStatusNotOk) {
    hzcc::StatusOr<int*> source(absl::CancelledError());
    hzcc::StatusOr<int*> target;
    target = source;
    EXPECT_EQ(target.status().code(), hzcc::StatusCode::kCancelled);
}

TEST(StatusOr, TestPointerAssignmentStatusOKConverting) {
    Derived derived;
    hzcc::StatusOr<Derived*> source(&derived);
    hzcc::StatusOr<Base2*> target;
    target = source;
    EXPECT_OK(target.status());
...
TEST(StatusOr, TestPointerAssignmentStatusNotOkConverting) {
    hzcc::StatusOr<Derived*> source(absl::CancelledError());
    hzcc::StatusOr<Base2*> target;
    target = source;
...
TEST(StatusOr, TestPointerStatus) {
    const int kI = 0;
    hzcc::StatusOr<const int*> good(&kI);
    EXPECT_TRUE(good.ok());
    hzcc::StatusOr<const int*> bad(absl::CancelledError());
    EXPECT_EQ(bad.status().code(), hzcc::StatusCode::kCancelled);
}

TEST(StatusOr, TestPointerValue) {
    const int kI = 0;
    hzcc::StatusOr<const int*> thing(&kI);
    EXPECT_EQ(&kI, *thing);
}

TEST(StatusOr, TestPointerValueConst) {
    const int kI = 0;
    const hzcc::StatusOr<const int*> thing(&kI);
    EXPECT_EQ(&kI, *thing);
}
...
TEST(StatusOr, StatusOrVectorOfUniquePointerCanReserveAndResize) {
    ...
    std::vector<::hzcc::StatusOr<EvilType>> v(5);
...
TEST(StatusOr, ConstPayload) {
    ...
    hzcc::StatusOr<const int> a;

    // Copy-construction
    hzcc::StatusOr<const int> b(a);

    // Copy-assignment
    EXPECT_FALSE(std::is_copy_assignable<hzcc::StatusOr<const int>>::value);

    // Move-construction
    hzcc::StatusOr<const int> c(std::move(a));

    // Move-assignment
    EXPECT_FALSE(std::is_move_assignable<hzcc::StatusOr<const int>>::value);
}

TEST(StatusOr, MapToStatusOrUniquePtr) {
    // A reduced version of a problematic type found in the wild. All of the
    // operations below should compile.
    using MapType = std::map<std::string, hzcc::StatusOr<std::unique_ptr<int>>>;
...
TEST(StatusOr, ValueOrOk) {
    const hzcc::StatusOr<int> status_or = 0;
    EXPECT_EQ(status_or.value_or(-1), 0);
}

TEST(StatusOr, ValueOrDefault) {
    const hzcc::StatusOr<int> status_or = absl::CancelledError();
    EXPECT_EQ(status_or.value_or(-1), -1);
}

TEST(StatusOr, MoveOnlyValueOrOk) {
    EXPECT_THAT(hzcc::StatusOr<std::unique_ptr<int>>(std::make_unique<int>(0))
                    .value_or(std::make_unique<int>(-1)),
                Pointee(0));
}

TEST(StatusOr, MoveOnlyValueOrDefault) {
    EXPECT_THAT(hzcc::StatusOr<std::unique_ptr<int>>(absl::CancelledError())
                    .value_or(std::make_unique<int>(-1)),
                Pointee(-1));
}

static hzcc::StatusOr<int> MakeStatus() { return 100; }

TEST(StatusOr, TestIgnoreError) { MakeStatus().IgnoreError(); }

TEST(StatusOr, EqualityOperator) {
    constexpr size_t kNumCases = 4;
    std::array<hzcc::StatusOr<int>, kNumCases> group1 = {
        hzcc::StatusOr<int>(1), hzcc::StatusOr<int>(2),
        hzcc::StatusOr<int>(absl::InvalidArgumentError("msg")),
        hzcc::StatusOr<int>(absl::InternalError("msg"))};
    std::array<hzcc::StatusOr<int>, kNumCases> group2 = {
        hzcc::StatusOr<int>(1), hzcc::StatusOr<int>(2),
        hzcc::StatusOr<int>(absl::InvalidArgumentError("msg")),
        hzcc::StatusOr<int>(absl::InternalError("msg"))};
...
struct StatusOrConversionBase<T, ConvTraits::kImplicit> {
    operator hzcc::StatusOr<T>() const& {  // NOLINT
        return absl::InvalidArgumentError("conversion to hzcc::StatusOr");
    }
    operator hzcc::StatusOr<T>() && {  // NOLINT
        return absl::InvalidArgumentError("conversion to hzcc::StatusOr");
    }
};
...
struct StatusOrConversionBase<T, ConvTraits::kExplicit> {
    explicit operator hzcc::StatusOr<T>() const& {
        return absl::InvalidArgumentError("conversion to hzcc::StatusOr");
    }
    explicit operator hzcc::StatusOr<T>() && {
        return absl::InvalidArgumentError("conversion to hzcc::StatusOr");
    }
};
...
// This class has conversion operator to `hzcc::Status` based on the value of
// `conv_traits`.
...
struct StatusConversionBase<ConvTraits::kImplicit> {
    operator hzcc::Status() const& {  // NOLINT
        return absl::InternalError("conversion to Status");
    }
    operator hzcc::Status() && {  // NOLINT
        return absl::InternalError("conversion to Status");
    }
};
...
struct StatusConversionBase<ConvTraits::kExplicit> {
    explicit operator hzcc::Status() const& {  // NOLINT
        return absl::InternalError("conversion to Status");
    }
    explicit operator hzcc::Status() && {  // NOLINT
        return absl::InternalError("conversion to Status");
    }
};
...
// This class conditionally has conversion operator to `hzcc::Status`, `T`,
...
// 1. T/Status
// 2. U that has conversion operator to hzcc::StatusOr<T>
// 3. U that is convertible to Status
// 4. U that is convertible to T
TEST(StatusOr, ConstructionFromT) {
    // Construct hzcc::StatusOr<T> from T when T is convertible to
    // hzcc::StatusOr<T>
    {
        ConvertibleToAnyStatusOr v;
        hzcc::StatusOr<ConvertibleToAnyStatusOr> statusor(v);
    ...
    // Construct hzcc::StatusOr<T> from T when T is explicitly convertible to
    // Status
    {
        CustomType<MyType, kConvToStatus | kConvExplicit> v;
        hzcc::StatusOr<CustomType<MyType, kConvToStatus | kConvExplicit>> statusor(
            v);
        EXPECT_TRUE(statusor.ok());
    }
    {
...
// Construct hzcc::StatusOr<T> from U when U is explicitly convertible to T
TEST(StatusOr, ConstructionFromTypeConvertibleToT) {
    {
        CustomType<MyType, kConvToT | kConvExplicit> v;
        hzcc::StatusOr<MyType> statusor(v);
        EXPECT_TRUE(statusor.ok());
    }
    {
...
// Construct hzcc::StatusOr<T> from U when U has explicit conversion operator to
// hzcc::StatusOr<T>
TEST(StatusOr, ConstructionFromTypeWithConversionOperatorToStatusOrT) {
    {
        CustomType<MyType, kConvToStatusOr | kConvExplicit> v;
        hzcc::StatusOr<MyType> statusor(v);
        EXPECT_EQ(statusor, v.operator hzcc::StatusOr<MyType>());
    }
    {
...
TEST(StatusOr, ConstructionFromTypeConvertibleToStatus) {
    // Construction fails because conversion to `Status` is explicit.
    {
        ...
        EXPECT_EQ(statusor.status(), static_cast<hzcc::Status>(v));
    }
    {
...
TEST(StatusOr, AssignmentFromT) {
    // Assign to hzcc::StatusOr<T> from T when T is convertible to
    // hzcc::StatusOr<T>
    {
        ConvertibleToAnyStatusOr v;
        hzcc::StatusOr<ConvertibleToAnyStatusOr> statusor;
        statusor = v;
        EXPECT_TRUE(statusor.ok());
    }
    // Assign to hzcc::StatusOr<T> from T when T is convertible to Status
    {
        CustomType<MyType, kConvToStatus> v;
        hzcc::StatusOr<CustomType<MyType, kConvToStatus>> statusor;
        statusor = v;
        EXPECT_TRUE(statusor.ok());
    }
}
...
TEST(StatusOr, AssignmentFromTypeConvertibleToT) {
    // Assign to hzcc::StatusOr<T> from U when U is convertible to T
    {
        CustomType<MyType, kConvToT> v;
        hzcc::StatusOr<MyType> statusor;
        statusor = v;
        EXPECT_TRUE(statusor.ok());
    }
}
...
TEST(StatusOr, AssignmentFromTypeWithConversionOperatortoStatusOrT) {
    // Assign to hzcc::StatusOr<T> from U when U has conversion operator to
    // hzcc::StatusOr<T>
    {
        CustomType<MyType, kConvToStatusOr> v;
        hzcc::StatusOr<MyType> statusor;
...
TEST(StatusOr, AssignmentFromTypeConvertibleToStatus) {
    // Assign to hzcc::StatusOr<T> from U when U is convertible to Status
    {
        CustomType<MyType, kConvToStatus> v;
        hzcc::StatusOr<MyType> statusor;
        statusor = v;
...
#endif  // HZCC_STATUS_OR_INTERNAL_H
