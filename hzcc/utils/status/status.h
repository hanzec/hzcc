...
//
//   * An `hzcc::Status` class for holding error handling information
//   * A set of canonical `hzcc::StatusCode` error codes, and associated
//     utilities for generating and propagating status codes.
//   * A set of helper functions for creating status codes and checking their
//     values
//
// Within Google, `hzcc::Status` is the primary mechanism for communicating
...
// should be designed to return an `hzcc::Status` (or `hzcc::StatusOr`).
//
// Example:
//
// hzcc::Status myFunction(std::string_view fname, ...) {
//   ...
//   // encounter error
//   if (error condition) {
...
// }
//
// An `hzcc::Status` is designed to either return "OK" or one of a number of
// different error codes, corresponding to typical error conditions.
// In almost all cases, when using `hzcc::Status` you should use the canonical
// error codes (of type `hzcc::StatusCode`) enumerated in this header file.
...
// hzcc::StatusCode
//
// An `hzcc::StatusCode` is an enumerated type indicating either no error ("OK")
// or an error condition. In most cases, an `hzcc::Status` indicates a
// recoverable error, and the purpose of signalling an error is to indicate what
// action to take in response to that error. These error codes map to the proto
// RPC error codes indicated in https://cloud.google.com/apis/design/errors.
//
// The errors listed below are the canonical errors associated with
// `hzcc::Status` and are used throughout the codebase. As a result, these
...
//
// If your error handling code requires more context, you can attach payload
// to your status. See `hzcc::Status::SetPayload()` and
// `hzcc::Status::GetPayload()` below.
enum class StatusCode : int {
    // StatusCode::kOk
    //
    // kOK (gRPC code "OK") does not indicate an error; this value is returned
    // on
    // success. It is typical to check for this value before proceeding on any
    // given call across an API or RPC boundary. To check this value, use the
    // `hzcc::Status::ok()` member function rather than inspecting the raw code.
    kOk = 0,
    ...
    // general, more specific errors should be raised, if possible. Errors
    // raised
    ...
    // specified an invalid argument, such as a malformed filename. Note that
    // use
    // of such errors should be narrowly limited to indicate the invalid nature
    // of
    // the arguments themselves. Errors with validly formed arguments that may
    // cause errors with the state of the receiving system should be denoted
    // with
    ...
    // expired before the operation could complete. For operations that may
    // change
    // state within a system, this error may be returned even if the operation
    // has
    // completed successfully. For example, a successful response from a server
    // could have been delayed long enough for the deadline to expire.
    kLexicalAnalysisStage = 4,
    ...
    //
    // `kNotFound` is useful if a request should be denied for an entire class
    // of
    ...
    // caller
    // does not have permission to execute the specified operation. Note that
    // this
    ...
    //      level (such as when a client-specified test-and-set fails,
    //      indicating
    //      the client should restart a read-modify-write sequence).
    //  (c) Use `kFailedPrecondition` if the client should not retry until
    //      the system state has been explicitly fixed. For example, if a
    //      "rmdir"
    ...
    // such as this can be corrected by retrying with a backoff scheme. Note
    // that
...
// hzcc::StatusToStringMode
//
// An `hzcc::StatusToStringMode` is an enumerated type indicating how
// `hzcc::Status::ToString()` should construct the output string for a non-ok
// status.
enum class StatusToStringMode : int {
    // ToString will not contain any extra data (such as payload). It will only
    // contain the error code and message, if any.
    kWithNoExtraData = 0,
    // ToString will contain the payload.
    kWithPayload = 1 << 0,
    // ToString will include all the extra data this Status has.
    kWithEverything = ~kWithNoExtraData,
    // Default mode used by ToString. Its exact value might change in the
    // future.
    kDefault = kWithPayload,
};

// hzcc::StatusToStringMode is specified as a bitmask type, which means the
...
// hzcc::Status
//
// The `hzcc::Status` class is generally used to gracefully handle errors
// across API boundaries (and in particular across RPC boundaries). Some of
// these errors may be recoverable, but others may not. Most
// functions which can produce a recoverable error should be designed to return
// either an `hzcc::Status` (or the similar `hzcc::StatusOr<T>`, which holds
// either an object of type `T` or an error).
//
// API developers should construct their functions to return `absl::NoError()`
// upon success, or an `hzcc::StatusCode` upon another type of error (e.g
// an `hzcc::StatusCode::kInvalidArgument` error). The API provides convenience
...
//   // encounter error
//   if (error condition) {
//     // Construct an hzcc::StatusCode::kInvalidArgument error
//     return absl::InvalidArgumentError("bad mode");
//   }
//   // else, return OK
//   return absl::NoError();
...
// Example:
//
//   hzcc::Status result = DoSomething();
//   if (!result.ok()) {
//     LOG(ERROR) << result;
//   }
//
...
//     case hzcc::StatusCode::kPermissionDenied:
//       LOG(ERROR) << result;
//       break;
...
//   }
//
// An `hzcc::Status` can optionally include a payload with more information
...
//     // other payload), and an `absl::Cord` to hold the encoded data.
...
class Status final {
  public:
    ...
    // `hzcc::StatusCode` and error message.  If `code ==
    // hzcc::StatusCode::kOk`,  // NOLINT `msg` is ignored and an object
    // identical to an OK status is constructed.
    //
    // The `msg` string must be in UTF-8. The implementation may complain (e.g.,
    // // NOLINT by printing a warning) if it is not.
    Status(hzcc::StatusCode code, std::string_view msg);
    ...
    // If the existing status already contains a non-OK error, this update has
    // no effect and preserves the current data. Note that this behavior may
    // change in the future to augment a current non-ok status with additional
    ...
    // Status::ok()
    //
    // Returns `true` if `this->code()` == `hzcc::StatusCode::kOk`,
    ...
    // Status::code()
    //
    // Returns the canonical error code of type `hzcc::StatusCode` of this
    // status.
    [[nodiscard]] hzcc::StatusCode code() const;
    ...
    // `google.rpc.Code` definitions within
    // https://github.com/googleapis/googleapis/blob/master/google/rpc/code.proto.
    // These values could be out of the range of canonical `hzcc::StatusCode`
    // enum values.
    //
    // NOTE: This function should only be called when converting to an
    // associated wire format. Use `Status::code()` for error handling.
    int raw_code() const;
    ...
    // unusual for the error message to be the empty string. As a result, prefer
    ...
    // error that may not be satisfied by an existing `hzcc::StatusCode`.
    ...
    // passing it the type URL and an `absl::Cord` of associated data.
    // Similarly, to extract the payload from a status, call
    // `Status::GetPayload()`. You may attach multiple payload (with differing
    // type URLs) to any given status object, provided that the status is
    // currently exhibiting an error code (i.e. is not OK).

    // Status::GetPayload()
    //
    // Gets the payload of a status given its unique `type_url` key, if present.
    [[nodiscard]] std::optional<absl::Cord> GetPayload() const;

  private:
    // Creates a status in the canonical error space with the specified
    // code, and an empty error message.
    explicit Status(hzcc::StatusCode code);
    ...
    static uintptr_t CodeToInlinedRep(hzcc::StatusCode code);
    static hzcc::StatusCode InlinedRepToCode(uintptr_t rep);

    // Converts between StatusRep* and the external uintptr_t representation
    // used by rep_. See rep_ for details.
    ...
    //    The (rep_ & 2) bit is the "moved from" indicator, used in
    //    IsMovedFrom().
...
// usage of `absl::NoError()` when constructing such an OK status.
Status NoError();
...
// `hzcc::StatusCode` error code of its associated function.
...
inline Status::Status() : rep_(CodeToInlinedRep(hzcc::StatusCode::kOk)) {}

inline Status::Status(hzcc::StatusCode code) : rep_(CodeToInlinedRep(code)) {}
...
inline void swap(hzcc::Status& a, hzcc::Status& b) {
    using std::swap;
    swap(a.rep_, b.rep_);
}
...
inline uintptr_t Status::MovedFromRep() {
    return CodeToInlinedRep(hzcc::StatusCode::kInternal) | 2;
}

inline uintptr_t Status::CodeToInlinedRep(hzcc::StatusCode code) {
    return static_cast<uintptr_t>(code) << 2;
}

inline hzcc::StatusCode Status::InlinedRepToCode(uintptr_t rep) {
    assert(IsInlined(rep));
    return static_cast<hzcc::StatusCode>(rep >> 2);
}
...
inline Status NoError() { return Status(); }
}  // namespace hzcc
#endif  // HZCC_UTILS_STATUS_STATUS_H_