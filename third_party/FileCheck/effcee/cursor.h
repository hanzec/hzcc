...
class Cursor {
  public:
    explicit Cursor(StringPiece str) : remaining_(str), line_num_(1) {}

    [[nodiscard]] StringPiece remaining() const { return remaining_; }
    // Returns the current 1-based line number.
    [[nodiscard]] int line_num() const { return line_num_; }

    // Returns true if the remaining text is empty.
    [[nodiscard]] bool Exhausted() const { return remaining_.empty(); }

    // Returns a string piece from the current position until the end of the
    // line or the end of input, up to and including the newline.
    [[nodiscard]] StringPiece RestOfLine() const {
        const auto newline_pos = remaining_.find('\n');
        return remaining_.substr(
            0, newline_pos + (newline_pos != StringPiece::npos));
    }
