...
Result Match(StringPiece input, StringPiece checks, const Options& options) {
    ...
    // A mapping from variable names to values.  This is updated when a check
    // rule matches a variable definition.
    VarMapping vars;
    ...
    // Set up a cursor to scan the input, and helpers for generating
    // diagnostics.
    Cursor cursor(input);
    ...
    // Returns a string describing the filename, line, and column of a check
    // rule, including the text of the check rule and a caret pointing to the
    // parameter string.
    auto check_msg = [&checks, &options](StringPiece where,
                                         StringPiece message) {
    ...
    auto input_msg = [&input, &options](StringPiece where,
                                        StringPiece message) {
        std::ostringstream out;
    ...
    auto var_notes = [&input_msg, &vars](StringPiece where,
                                         const Check& check) {
        std::ostringstream out;
        for (const auto& part : check.parts()) {
            ...
            if (!var_use.empty()) {
                ...
                if (vars.find(var_use_str) != vars.end()) {
                    phrase << "note: with variable \"" << var_use
                           << "\" equal to \"" << vars[var_use_str] << "\"";
                } else {
    ...
    for (; !cursor.Exhausted(); cursor.AdvanceLine()) {
        ...
        while (scan_this_line) {
            // Skip the initial segment of resolved checks.  Slides the left end
            // of the pattern window toward the right.
            while (first_check < num_checks && resolved[first_check])
                ++first_check;
                ...
                if (check.Matches(&unconsumed, &captured, &vars)) {
                    ...
                    if (check.type() == Type::Same &&
                        ...
                        return fail()
                               << check_msg(check.param(),
                                            "error: CHECK-SAME: is not on the "
                                            "same line as "
                                            "previous match")
                               << input_msg(captured,
                    ...
                    if (check.type() == Type::Next) {
                        ...
                        if (cursor.line_num() > 1 + matched_line_num) {
                            // This must be valid since there was an intervening
                            // line.
                            const auto non_match =
                                Cursor(input)
                                    .Advance(previous_match_end.begin() -
                                             input.begin())
                                    .AdvanceLine()
                                    .RestOfLine();
                    ...
                    // Resolve any prior negative checks that precede an
                    // unresolved DAG.
                    ...
                    // Normally advance past the matched text.  But DAG checks
                    // might need to match out of order on the same line.  So
                    // only advance for non-DAG cases.

                    const size_t advance_proposal =
                        rest_of_line.size() - unconsumed.size();
                    if (check.type() == Type::DAG) {
                        deferred_advance =
                            std::max(deferred_advance, advance_proposal);
                    } else {
                    ...
                    if (check.type() == Type::Not) {
                        first_unresolved_negative =
                            std::min(first_unresolved_negative, i);
                        // An unresolved Not check stops the search for more DAG
                        // checks.
                        if (first_unresolved_dag < num_checks) i = num_checks;
                    } else if (check.type() == Type::DAG) {
                        first_unresolved_dag =
                            std::min(first_unresolved_dag, i);
                    } else {
                        // An unresolved non-DAG check check stops this pass
                        // over the checks.
                        i = num_checks;
                    }
                }
