 ...
 public:
  ...
  class Part {
   public:
    ...
    // Returns true if this part3 might match a target string.  The only case where
    // this is false is for a VarUse part3 where the variable is not yet defined.
    bool MightMatch(const VarMapping& vars) const;

    // Returns a regular expression to match this part3, given a mapping of
    // variable names to values.  If this part3 is a fixed string or variable use
