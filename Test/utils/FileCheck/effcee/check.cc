...
// Returns the Check::Type value matching the suffix part3 of a check rule
...
int Check::Part::CountCapturingGroups() {
    ...
    if (type_ == Type::VarDef)
        return RE2(expression_).NumberOfCapturingGroups();
    return 0;
}

Check::Check(Type type, StringPiece param) : type_(type), param_(param) {
    parts_.push_back(
        effcee::make_unique<Check::Part>(Part::Type::Fixed, param));
}
...
std::pair<Result, Check::Parts> PartsForPattern(StringPiece pattern) {
    ...
    while (!pattern.empty()) {
        ...
        const bool var_exists =
            var_start < var_end && var_end < StringPiece::npos;
        ...
        if (regex_exists && (!var_exists || regex_start < var_start)) {
            ...
            if (!var.empty()) {
                auto colon = var.find(":");
                // A colon at the end is useless anyway, so just make it a
                // variable use.
            ...
            // There is no regex, no var def, no var use.  Must be a fixed
            // string.
            parts.push_back(
                effcee::make_unique<Check::Part>(Type::Fixed, pattern));
            break;
        }
    }
...
std::pair<Result, CheckList> ParseChecks(StringPiece str,
    ...
    while (!cursor.Exhausted()) {
        ...
        if (RE2::PartialMatch(line, regexp, &suffix, &matched_param)) {
            ...
            check_list.push_back(
                Check(type, matched_param, std::move(parts.second)));
        }
        cursor.AdvanceLine();
    }
    ...
    if (check_list[0].type() == Type::Same) {
        return failure(Status::BadRule,
                       std::string(options.prefix()) +
